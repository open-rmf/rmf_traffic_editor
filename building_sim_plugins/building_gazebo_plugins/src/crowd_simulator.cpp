  ...
  const gazebo::common::UpdateInfo& update_info)
{
  ...
  {
    if (crowd_sim_interface->_gz_last_sim_time == gazebo::common::Time::Zero)
      crowd_sim_interface->_gz_last_sim_time = update_info.simTime;
  }
  ...
  {
    auto delta_sim_time =
      (update_info.simTime - crowd_sim_interface->_gz_last_sim_time).Double();
    ...
    {
      for (size_t id = 0; id < crowd_sim_interface->get_num_objects(); id++)
      {
        auto name = crowd_sim_interface->get_internal_agent_name(id);
  ...
  const crowd_simulator::ModelTypeDatabase::RecordPtr type_ptr)
{
  ...
  auto original_z = actor_ptr->WorldPose().Pos().Z();
