...
//=======================================
std::shared_ptr<AgentGroupTab> AgentGroupTab::init_and_make(CrowdSimImplPtr crowd_sim_impl)
{
    const QStringList labels = 
    { "id", "profile", "initial state", "spawn number", 
      "external agent", "point_x", "point_y", ""};
    
    auto agent_group_tab = std::make_shared<AgentGroupTab>(crowd_sim_impl, labels);
    if (!agent_group_tab) 
    {
...
{
  if (row_number == 0) {
      std::cout << "Default agent_group for external agents is not allowed to be deleted. " << std::endl;
      return;
  }
