 ...
 :_current_transition(transition)
{
    ...
    QHBoxLayout* root_condition_value = new QHBoxLayout(_root_condition_value_container);
    ...
    _construct_leaf_condition_widget(_condition1_container, _condition1_type, _condition1_value, 1);
    _construct_leaf_condition_widget(_condition2_container, _condition2_type, _condition2_value, 2);
...
void ConditionDialog::update()
{
    ...
    if (Condition::BASE == root_condition->get_type()) return;

    if (Condition::GOAL == root_condition->get_type() || 
        Condition::TIMER == root_condition->get_type() )
    {
        auto root_condition_tmp = std::dynamic_pointer_cast<crowd_sim::LeafCondition>(root_condition);
        _rootValueD = root_condition_tmp->get_value();
        _root_value->setText(QString::number(_rootValueD));
        return;
    }

    auto root_condition_tmp = std::dynamic_pointer_cast<crowd_sim::BoolCondition>(root_condition);
    auto sub_condition_1 = root_condition_tmp->get_condition(1);
    _condition1ValueD = std::dynamic_pointer_cast<crowd_sim::LeafCondition>(sub_condition_1)->get_value();
    ...
    _condition2ValueD = std::dynamic_pointer_cast<crowd_sim::LeafCondition>(sub_condition_2)->get_value();
    ...
    connect(
        ...
        [this](int index) {
            switch (index) 
            {
                case 1 :
                    _root_is_leaf_condition();
                    if(_current_transition.get_condition()->get_type() != crowd_sim::Condition::GOAL)
                        _current_transition.set_condition(std::make_shared<crowd_sim::ConditionGOAL>() );
                    break;
                case 2 :
                    _root_is_leaf_condition();
                    if(_current_transition.get_condition()->get_type() != crowd_sim::Condition::TIMER)
                        _current_transition.set_condition(std::make_shared<crowd_sim::ConditionTIMER>() );
                    break;
                case 3 :
                    _root_is_bool2_condition();
                    if(_current_transition.get_condition()->get_type() != crowd_sim::Condition::AND)
                        _current_transition.set_condition( std::make_shared<crowd_sim::ConditionAND>() );
                    break;
                case 4 :
                    _root_is_bool2_condition();
                    if(_current_transition.get_condition()->get_type() != crowd_sim::Condition::OR)
                        _current_transition.set_condition( std::make_shared<crowd_sim::ConditionOR>() );
                    break;
                case 5 :
                    _root_is_not_condition();
                    if(_current_transition.get_condition()->get_type() != crowd_sim::Condition::NOT)
                        _current_transition.set_condition( std::make_shared<crowd_sim::ConditionNOT>() );
                    break;
                default :
                    _root_is_invalid();
            }
    ...
    int condition_index)
{
    ...
    if (root_condition->get_type() == Condition::NOT) 
        condition_index = 1;
    auto bool_root_condition = std::dynamic_pointer_cast<crowd_sim::BoolCondition>(root_condition);
