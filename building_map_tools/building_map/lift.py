...
class Lift:
    def __init__(self, yaml_node, name, transform):
        ...
        self.cabin_f_max = \
            (self.cabin_mass + self.cabin_max_load) \
            * (self.cabin_a_max + 9.81)    # max force
    ...
    def generate_cabin(self, world_ele):
        ...
        lift_cabin_model_name = f'LiftCabin_{self.name}'
        ...
        back_wall_pose.text = f'{-self.depth / 2 + 0.025} 0 1.25 0 0 0'
        platform.append(
            generate_visual(
                back_wall_name,
                back_wall_pose,
        ...
        left_wall_pose.text = f'0 {self.width / 2 - 0.025} 1.25 0 0 0'
        platform.append(
            generate_visual(
                left_wall_name,
                left_wall_pose,
        ...
        right_wall_pose.text = f'0 {-self.width / 2 + 0.025} 1.25 0 0 0'
        platform.append(
            generate_visual(
                right_wall_name,
                right_wall_pose,
        ...
        front_left_wall_pose.text = \
            f'{self.depth / 2 - 0.025} {self.width * 3 / 8} 1.25 0 0 0'
        platform.append(
            generate_visual(
        ...
        left_door_pose = Element('pose')
        left_door_pose.text = \
            f'{self.depth / 2 - 0.025} {self.width / 8} 1.25 0 0 0'
        lift_cabin_model.append(
            generate_box_link(
                'left_door',
                door_size,
