...
class ConnectionManager:
    ...
    # check the generated polygon vertices (vertex0 and vertex1) is on the same side of lane
    ...
    # calculate the polygon vertex within the area from vector0 to vector1, mind the sequence
    def cal_polygon_vertex_from_lane(self, id0, id1):
        lane0 = self.lane_manager.data[id0]
        lane1 = self.lane_manager.data[id1]
        base_vertex_id_intersect = list( set(lane0.lane_vertex_id) & set(lane1.lane_vertex_id) )
        ...
        if(abs( vector0.get_dot(vector1.get_normal_unit()) ) < 0.05) :
            ...
            # the polygon vertex must be within the area from vertor0 to vector1
